...
def _to_ws_code(code: str) -> str:
    ...
    **单一事实来源**：直接委托 ``westock_helpers._to_ws``，不再维护第二份
    路由逻辑。历史上 westock_cli 与 westock_helpers 各有一份，导致 ETF
    前缀 bug 只修了一处、另一处漏修（见 git 历史 3174c98 / 552a263）。
    委托后，路由规则只存在于一处，杜绝漂移。
    """
    from westock_helpers import _to_ws
    return _to_ws(code)
